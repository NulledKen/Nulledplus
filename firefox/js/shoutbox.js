					...
					if(shoutID != "shout_global_srv_msg") {
						...
						if(check_blacklist_users(shoutAuthor) || check_blacklist_words(shoutMessage.text()))
							...
							if(!$(this).contents().find("img.block_user").length) {
								...
								$(this).children().eq(0).children().eq(0).before("<a name='" + shoutAuthor + "' class='block_user' title='Block " + shoutAuthor + "'><img src='" + browser.extension.getURL("images/block_icon.png") + "' class='block_user'></a>");
							}
							// Tags in message
								...
								if($(this).attr("href").split("#")[0].split("!")[1] == userID) {
									...
									if($("input#sound_on_tag").prop('checked')) {
										if(!vis()) {
											...
											notification.src = browser.extension.getURL('sounds/notification.mp3');
											notification.play();
										}
									}
								}
